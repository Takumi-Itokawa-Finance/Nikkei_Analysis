...
cells.append(code([
    ...
    "plt.show()\n",
    "\n",
    "print('\\nCorrelation with Open Return (descending):')\n",
...
cells.append(code([
    ...
    "\n",
    "idea1_rows = [\n",
    "    eval_metrics(feat['open_ret'],     synthetic_ret, 'Open open_ret'),\n",
    "    eval_metrics(feat['close_ret'],    synthetic_ret, 'Close close_ret'),\n",
    "    eval_metrics(feat['intraday_ret'], synthetic_ret, 'Intraday intraday_ret'),\n",
    "]\n",
...
cells.append(code([
    ...
    "    ['Open Return (open_ret)', 'Close Return (close_ret)', 'Intraday Return (intraday_ret)'],\n",
...
cells.append(code([
    ...
    "ax.set_title('Idea 1: Open Return — Actual vs Synthetic')\n",
    ...
    "        linestyle='--', label='Synthetic (rule-based)')\n",
    "ax.set_ylabel('Return (%)')\n",
    "ax.set_title('Idea 1: Close Return — Actual vs Synthetic')\n",
    "ax.legend(fontsize=9)\n",
    "ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m'))\n",
    "\n",
    "plt.tight_layout()\n",
...
cells.append(code([
    ...
    "TARGET_LABELS = {\n",
    "    'open_ret':     'Open Return',\n",
    "    'close_ret':    'Close Return',\n",
    "    'intraday_ret': 'Intraday Return',\n",
    "}\n",
    "\n",
...
cells.append(code([
    "# -- Feature importance: Ridge coefficients (Open and Close return) -------\n",
    "fig, axes = plt.subplots(1, 2, figsize=(14, 5))\n",
    "\n",
    "for ax, tcol, tlabel in zip(\n",
    "    axes,\n",
    "    ['open_ret', 'close_ret'],\n",
    "    ['Open Return (open_ret)', 'Close Return (close_ret)'],\n",
    "):\n",
    "    coef = pd.Series(\n",
...
cells.append(code([
    "# -- XGBoost feature importance (Open and Close return) ------------------\n",
    "fig, axes = plt.subplots(1, 2, figsize=(14, 5))\n",
    "\n",
    "for ax, tcol, tlabel in zip(\n",
    "    axes,\n",
    "    ['open_ret', 'close_ret'],\n",
    "    ['Open Return', 'Close Return'],\n",
    "):\n",
    "    imp = pd.Series(\n",
...
cells.append(code([
    ...
    "\n",
    "# unstack() sorts columns alphabetically; reindex to the intended order\n",
    "# (Open Return, Close Return, Intraday Return) before extracting .values.\n",
    "target_order = list(TARGET_LABELS.values())  # ['Open Return', 'Close Return', 'Intraday Return']\n",
    "pivot = pivot.reindex(columns=target_order)\n",
    "\n",
    "row1 = pivot.loc['Idea 1 (Rule-based)'].values\n",
    "row2 = pivot.loc['Idea 2 (XGBoost)'].values\n",
    "\n",
    "fig, ax = plt.subplots(figsize=(10, 5))\n",
    "x = np.arange(len(row1))\n",
    "w = 0.35\n",
    "bar1 = ax.bar(x - w/2, row1, w, label='Idea 1 (Rule-based)', color='steelblue', alpha=0.85)\n",
    "bar2 = ax.bar(x + w/2, row2, w, label='Idea 2 (XGBoost)',   color='orange',    alpha=0.85)\n",
    "\n",
...
out_path = r'C:\Users\admin\Nikkei_Analysis\notebooks\06_adr_prediction.ipynb'
with open(out_path, 'w', encoding='utf-8') as f:
    json.dump(nb, f, indent=1, ensure_ascii=False)
print('06_adr_prediction.ipynb created')
